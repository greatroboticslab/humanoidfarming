...
def make_example(tokenizer, image_processor, example):
    ...
    # ⚠️ IMPORTANT CHANGE:
    # Ignore noisy per-frame input_text / transcripts.
    # Train the model as an image-captioner: image -> one descriptive sentence.
    prompt = (
        "You are an assistant that describes ONLY what is visible in a single farming-related video frame. "
        "Do not guess information that is not clearly visible. "
        "Describe the frame in one short, clear, detailed sentence that helps understand the farming or product.\n\n"
        "Answer: "
    )
    ...
    labels[:, :prompt_len] = -100  # ignored by CrossEntropyLoss
