...
FRAME_ROOT = Path("results/frame_extractions")

# Where to write one JSON per video with tasks + subtasks + frames + captions
JSON_OUT_DIR = Path("results/frame_captions2")

# Where the tasks-with-timestamps JSONs live
TASK_JSON_DIR = Path("results/tasks_with_timestamps")
...
def process_video_frames(video_dir: Path, processor, model):
    ...
    # Build final payload: keep original metadata + updated tasks structure
    out_payload = {
