...
DEFAULT_FRAME_CAPTION_DIR = "results/unified_pipeline/frame_captions"
FRAME_CAPTION_DIR = Path(os.environ.get("FRAME_CAPTION_DIR", DEFAULT_FRAME_CAPTION_DIR))
OUTPUT_DIR = Path(os.environ.get("OUTPUT_DIR", "results/unified_pipeline/subtask_guidance"))
...
def build_subtask_prompt(
    ...
    # Pipeline 2 context preserved by the integration bridge.
    pipeline2_task_ctx = task_obj.get("pipeline2") or {}
    pipeline2_sub_ctx = subtask_obj.get("pipeline2_ref") or {}

    mission_title = (
        pipeline2_task_ctx.get("mission_title")
        or pipeline2_sub_ctx.get("mission_title")
        or title
        or ""
    )

    sub_mission_title = (
        pipeline2_task_ctx.get("sub_mission_title")
        or pipeline2_sub_ctx.get("sub_mission_title")
        or ""
    )

    dominant_category = (
        pipeline2_task_ctx.get("dominant_category")
        or pipeline2_sub_ctx.get("dominant_category")
        or ""
    )

    original_task_text = (
        pipeline2_sub_ctx.get("original_task_text")
        or task_text
        or ""
    )
...
VIDEO / MISSION CONTEXT
-----------------------
- video_index: {video_index}
- video_title: {title}
- mission_title: {mission_title}
- sub_mission_title: {sub_mission_title}
- high_level_task: {original_task_text}
- pipeline2_category: {dominant_category}
...
----------------------
Use the mission and sub-mission as big-picture context.
Still generate concrete robot guidance for the specific subtask only.
Do not ignore the subtask timestamp and frame captions.
